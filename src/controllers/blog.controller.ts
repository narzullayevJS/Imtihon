...
export const createBlog = async (req: Request, res: Response) => {
  try {
    ...
    const newBlog = await pool.query(
      "INSERT INTO blogs (title, description, owner_id) VALUES ($1, $2, $3) RETURNING *",
...
export const getMyBlogs = async (req: Request, res: Response) => {
  try {
    ...
    const blogs = await pool.query("SELECT * FROM blogs WHERE owner_id = $1", [userId])
...
export const updateBlog = async (req: Request, res: Response) => {
  try {
    ...
    const updatedBlog = await pool.query("UPDATE blogs SET title = $1, description = $2 WHERE id = $3 RETURNING *", [
      title,
      description,
      id,
    ])
...
export const searchBlogs = async (req: Request, res: Response) => {
  try {
    ...
    const blogs = await pool.query("SELECT * FROM blogs WHERE title ILIKE $1", [`%${query}%`])
...
export const getBlogUsers = async (req: Request, res: Response) => {
  try {
    ...
    const users = await pool.query(
      "SELECT u.id, u.username, u.email FROM users u JOIN blog_members bm ON u.id = bm.user_id WHERE bm.blog_id = $1",
      [id],
    )
